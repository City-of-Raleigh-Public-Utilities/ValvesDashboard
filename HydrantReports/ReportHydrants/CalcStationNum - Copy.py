...
try:
    ...
    hydFc = os.path.join(sde,"RPUD.ALL_HYDRANTS")
    ...
    for grid in gridCur:
        ...
        arcpy.SelectLayerByAttribute_management("hyd_layer","SUBSET_SELECTION","STATION IS NULL")
        cnt = int(str(arcpy.GetCount_management("hyd_layer")))
        print str(cnt)
        if cnt > 0:
            arcpy.CalculateField_management("hyd_layer", "STATION",grid.STATION)
    del grid
    del gridCur
except:
